                ...
                <div class="card-body">
                    ...
                    <table class="table datatable">
                        ...
                        <tbody>
                            <?php
                            $id = $_COOKIE["view_id"];
                            $stmt = $obj->con1->prepare("SELECT c1.id, c1.case_no, ch.remarks, ch.status, date_format(ch.dos, '%d-%m-%Y') as fdos, date_format(ch.date_time, '%d-%m-%Y') as fdt, it.name as intern_name, s.stage as stage_name, c1.case_no, ad.name as advocate_name from `case_hist` as ch inner join `task` as t on t.id = ch.task_id inner join `case` c1 on c1.id = t.case_id inner join `stage` as s on ch.stage = s.id inner join `staff` as it on t.alloted_to = it.id inner join `staff` as ad on ad.id = t.alloted_by where t.case_id = ? order by ch.id DESC;");
                            $stmt->bind_param("i", $id);
                            $stmt->execute();
