    ...
    <div class="row">
        ...
        <div class="card">
             ...
             //               $stmt = $obj->con1->prepare("SELECT *, interns.name as intern_name ,stage.stage as stage_name from `case_hist` inner join `task` on task.id = case_hist.task_id inner join `case` on case.id = task.case_id inner join `stage` on case_hist.stage = stage.id inner join `interns` on task.alloted_to = interns.id where task.case_id = '$id' order by case_hist.id DESC");
                   $stmt = $obj->con1->prepare("SELECT *, date_format(`case_hist`.dos,'%d-%m-%Y') as fdos , date_format(`case_hist`.date_time,'%d-%m-%Y') as fdt , interns.name as intern_name ,stage.stage as stage_name , case.case_no from `case_hist` inner join `task` on task.id = case_hist.task_id inner join `case` on case.id = task.case_id inner join `stage` on case_hist.stage = stage.id inner join `interns` on task.alloted_to = interns.id inner join advocate on advocate.id = task.alloted_by where task.case_id = $id order by case_hist.id DESC");
                            $stmt->execute();
